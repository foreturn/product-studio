...
| 能力簇 | 命中条件 | 正文 |
|---|---|---|
| 应用结构、生命周期与构建治理 | 新建功能模块、整理目录与命名、评审模块职责和抽象复用、调整工程结构、状态分层与数据流边界、处理场景切换与进程终止后的恢复、引入或升级依赖、调整构建配置与签名分发、升级目标系统版本或适配新系统行为与设备形态 | [structure-delivery.md](principles/structure-delivery.md) |
