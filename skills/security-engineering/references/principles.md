...
专业约束正文按簇分卷存放于 `references/principles/`，每簇汇聚数个专业类别的跨技术栈不变量。先依命中条件判定本次任务命中的簇（通常一至三簇），再完整读取命中簇文件；命中存疑时宁可加读相邻簇，不因漏判而缺约束；与本次工作无关的簇不读取。设计新系统或新入口时与完成实现进入安全验证时，加读“威胁建模与对抗验证”卷。
...
| 密码学、秘密与隐私 | 选择加密算法与密钥分层、设计秘密存储与轮换销毁、划定隐私处理目的与数据最小化、实现访问导出更正与删除传播 | [crypto-privacy.md](references/principles/crypto-privacy.md) |
| 供应链与运行环境 | 治理依赖引入与构建流水线、固定制品身份与来源证明、分级处置漏洞告警、加固运行环境与安全基线 | [supply-chain-runtime.md](references/principles/supply-chain-runtime.md) |
