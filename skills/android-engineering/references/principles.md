...
| 能力簇 | 命中条件 | 正文 |
|---|---|---|
| 应用结构、生命周期与构建治理 | 新建功能模块、整理目录与命名、评审模块职责和抽象复用、调整工程结构、状态分层与数据流边界、处理配置变化与进程重建后的恢复、引入或升级依赖、调整构建变体与产物配置、升级 targetSdk 或适配新系统行为与设备形态 | [structure-delivery.md](principles/structure-delivery.md) |
