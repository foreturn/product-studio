...
- `SKILL.md` 的中文 `description` 只作专业发现摘要；正文统一保留目标、六步执行、输出、完成、停止、权限、参考和项目记忆八章。它不列适用、不适用或必需输入清单，不展开领域方法，也不编排其他 Skill。
- `references/principles.md` 是通用专业约束正文。正文按八至十六个专业大类组织，每类只列四至八条跨框架仍成立的不变量、取舍标准或禁止边界；不设置字段，不固化特定库、算法、架构模板、阈值或操作步骤。
- `references/memory.md` 只规定本 Owner 应记住哪些项目核心认知。每卷只有“使用方式”和“核心记忆”两章，并以五至九个专业主题列出会持续影响后续判断的项目不变量。
...
每项 Skill 都在 `references/memory.md` 中列明本专业应记住的核心认知，并在 `SKILL.md` 中明确要求：先唯一确认目标 Git 根与安全单级 `product-id`，开始相关工作前读取目标事实册，核心认知改变时更新或移除旧内容，只保留当前仍成立的事实。归属不唯一或名称不安全时不猜测或创建事实册；事实册不存在时直接依据当前仓库工作，不以缺少记忆阻断任务。
