...
- `SKILL.md` 的中文 `description` 只作专业发现摘要；正文统一保留目标、六步执行和项目记忆三章，其中“项目记忆”章独占当前产品根目录与产品标识定位、事实读取、准入、写权限解释、创建、维护、实例写法与排除规则。它不列适用、不适用或必需输入清单，不展开领域方法，也不编排其他 Skill。
- `references/principles.md` 是专业约束的瘦索引，约束正文按簇分卷存放于 `references/principles/`。索引只有“能力索引”一章：先以一段读取协议写明“先依命中条件判定本次任务命中的簇、命中存疑时宁可加读相邻簇、与本次工作无关的簇不读取”，再以三列表格列出四至八个能力簇的簇名、任务句式命中条件与分卷路径。能力簇以可独立命中的变化原因及其必须共同加载的记忆边界划分，不按类别数量凑数，也不把依赖不同的职责捆在同一卷；每卷约束按一至数个专业大类组织，全部簇卷合计八至十六个大类，每类只列四至八条跨框架仍成立的不变量、取舍标准或禁止边界。不设置字段，不固化特定库、算法、架构模板、阈值或操作步骤，未命中的簇卷不会被读取。
- `references/memory/` 只定义本 Owner 应记住哪些项目核心认知，按能力簇分卷存放，文件名与 `references/principles/` 对应簇卷文件名一致。每卷只有标题与若干条“记住……”陈述，只列会持续影响后续判断的项目不变量；命中哪簇便只读对应卷，不再承载使用协议。
...
每项 Skill 在作出、沿用或修复专业决定前，都先区分用户已裁定的目标与政策、当前事实、因果推断和待证假设。用户或编码代理给出的具体方案、既有实现与已有测试都不能因来源而自动成为正确结论；用户拥有目标、政策与取舍的裁决权，但方案仍须满足其依赖的事实、约束和验收结果。

方案的简便性按受影响完整流程衡量，而非只看当前文件或局部实现：从真实入口到可确认结果，一并审视必要状态与交接、失败恢复、规则是否重复、变更如何扩散，以及用户、开发和运行维护的总成本。在结果同样完整时选择总复杂度最低且易于理解的方案，不以极简之名省略必要边界。

异常出现后先区分实现偏离已确认决定、决定前提已被证伪和适用条件已经变化。若新证据推翻原决定前提，或修复只能依靠新增例外、复制规则或状态、绕过责任边界成立，便重审受影响决定及其验收，不在失效决定上继续叠加局部修补，也不因局部失败推翻无关决定；超出当前专业所有权时，只呈明矛盾、整体影响和待裁决选项，不自行改写其他专业语义。
...
十一位 Owner 的事实册目录与 Skill 名一致，事实按命中簇分文件存放：`<product-id>/<owner>/` 下每个 `.md` 文件对应一个命中簇，文件名与该 Skill `references/principles/` 中对应簇卷文件名一致，未命中的簇事实文件不读取。

事实册是语义索引，不是第二份源码。代码、权威 Schema、流水线、制品库和当前环境查询始终是真相来源；旧记忆与当前权威冲突时，以当前权威为准。

## 项目记忆

每项 Skill 都在 `references/memory/` 中按能力簇分卷列明本专业应记住的核心认知，并由 `SKILL.md` 的“项目记忆”章统一规定使用方式。事实册按簇分文件存放于当前产品根目录的 `docs/product-studio/<product-id>/<owner>/<能力簇卷文件名>.md`，且所有读写都从该根目录解析；开始相关工作前先读取命中簇的记忆卷与同名事实文件，证据闭环的核心认知即时落册，最终回复前再对本轮实际核验范围作一次强制收口对账。

凡任务已经允许修改当前产品根目录内的目标文件，维护该根目录下的事实册属于同一写权限，除非用户明确排除；只读分析、审查与状态查询仍保持只读。符合准入条件但尚未记录、已经改变或已经失效的事实，在证据闭环时即时写入对应簇的事实文件，到收口才闭环的（如整体构建或验证结果）在收口时直接创建、更新或移除，不得只在回复中列成候选。事实文件不存在且出现首条应入册事实时，在当前产品根目录下一并创建 `<product-id>/<owner>/` 目录与以簇卷文件名命名的 `.md`；没有事实时不创建空目录或空文件。本轮未修改、却由当前权威新确认的既有事实也必须纳入收口判断，符合准入条件便写入，记忆范围不等于 Git 差异。

事实文件首行固定为 `# <owner> 当前产品事实 · <簇名>`；每项事实以稳定、可检索的业务语义为二级标题，用现在时写清当前事实、成立条件、影响边界、相对于当前产品根目录的权威核验入口及失效或重审条件。同一认知只保留一处，旧内容原位更新或移除，不追加历史版本；簇事实文件内最后一条事实移除后删除该文件，`<product-id>/<owner>/` 目录为空时删除该目录，`<product-id>` 目录为空时一并删除。
