...
- `references/principles.md` 是专业约束的瘦索引，约束正文按簇分卷存放于 `references/principles/`。索引只有“能力索引”一章：先以一段读取协议写明“先依命中条件判定本次任务命中的簇、命中存疑时宁可加读相邻簇、与本次工作无关的簇不读取”，再以三列表格列出四至六个能力簇的簇名、任务句式命中条件与分卷路径。每卷约束按专业大类组织，全部簇卷合计八至十六个大类，每类只列四至八条跨框架仍成立的不变量、取舍标准或禁止边界；不设置字段，不固化特定库、算法、架构模板、阈值或操作步骤，未命中的簇卷不会被读取。
