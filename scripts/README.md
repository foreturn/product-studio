...
负向自检在临时副本中分别注入第六技能、router 记忆、缺失的设计能力卷、未声明的额外 reference、能力正文篡改、错误事实字段、重复事实键、畸形事实键、失去精确定位的源码锚点、错误模板骨架、旧调用名、把常规终端验证误算为跨领域，以及省略全栈 API 契约里程碑；13 类退化必须全部被拒绝，不改当前工作树。
