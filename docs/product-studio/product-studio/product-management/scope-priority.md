...
- **权威依据**：`README.md#十一项通用底座`；`.codex-plugin/plugin.json#keywords`；`skills/`；`.claude-plugin/plugin.json`
- **影响边界**：Codex 与 Claude Code 的技能发现、插件说明、专业 Owner 和项目事实 locator 均以此集合为准；数据工程与 AI 系统等条件专业不在当前通用底座内。
- **复核入口**：对照技能目录、frontmatter、双端清单和 README 核对专业集合与职责归属；新增、合并、删除或改名任何通用专业时重审。
