...
- **验证入口**：在插件根运行 `python3 -X utf8 scripts/validate_project.py --self-test`；关键断言是当前快照无结构错误、无实例事实册仍合法、39 类退化各自命中对应诊断且临时副本不改工作树，其中包括拒绝 router 重新取得终态裁决。该入口不证明事实语义已随最终代码更新，也不证明真实插件触发、目标项目运行或线上行为。
